...
struct Node* concatenateLists(struct Node* head1, struct Node* head2) {
    if (head1 == NULL) return head2;  //if first list is emplty then the result is just second list
    if (head2 == NULL) return head1;  //if second list is empty then the result is just first list

    struct Node* temp = head1;
    while (temp->next != NULL) {
        temp = temp->next;
    }
    temp->next = head2;   //connect last node of the list 1 to the first node of list2 
